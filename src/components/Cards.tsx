...
const Cards = () => {
    ...
    const cardsClass = `grid grid-cols-4 gap-[10px] text-[10px] p-[15px] mb-[15px] my-[5px] w-full mx-auto bg-[#FFFFFF] 
            border border-[#EAE9E2] shadow-sm break-words border-l-4 border-l-[#1E40AF]
            md:grid-cols-[40px_120px_80px_80px_180px_180px_110px_100px_120px_120px_100px_100px] 
            md:w-full md:max-w-none md:p-4 
            md:m-0 md:text-xs md:text-gray-700 
            md:rounded-none md:shadow-none md:bg-transparent
            md:border-0 md:border-b md:border-gray-200 md:gap-0`
    ...
    const baseMdClass = "md:col-auto md:row-auto"
    const violationDatesClass = "flex gap-[5px]"
    const buttonClass = "col-start-4 row-start-1 border border-[#1E40AF] rounded-lg font-bold w-[25px] h-[25px] ml-auto"
    // стили для рисков
        ...
        mockIssues.map((issue) => {
            return <div className={cardsClass}>
                ...
                <div className="flex items-baseline gap-2 col-start-1 col-end-4 row-start-1 md:contents">
                    <div className="px-1 font-bold text-gray-400">
                        <p className="md: hidden">{t('table.number')}</p>
                        <p><span className="md:hidden">№</span>{issue.id}</p>
                    </div>
                    <div className={`${baseMdClass} px-1`}>
                        <p className={inspectionClass}>{issue.inspection}</p>
                    </div>
                </div>
                {/* остальные колонки */}
                <div className="md:hidden col-start-1 col-end-2 row-start-2">
                    <p className={labelClass}>{t('table.violationPeriod')}</p>
                </div>
                <div className={`${baseMdClass} ${violationDatesClass} px-1 col-start-2 row-start-2`}>
                    <p className="md:hidden">c</p>
                    <p>{issue.startInspectionDate}</p>
                </div>
                <div className={`${baseMdClass} ${violationDatesClass} px-1 m-auto col-start-3 row-start-2`}>
                    <p className="md:hidden">по</p>
                    <p>{issue.endInspectionDate === null ? '-' : issue.endInspectionDate}</p>
                </div>
                <div className={`${baseMdClass} px-1 col-start-1 col-end-5 row-start-3`}>
                    <p className={violationClass}>{issue.auditViolation}</p></div>
                <div className={`${borderClass} ${baseMdClass} ${recommendationClass} px-1 col-start-1 col-end-5 row-start-5`}>
                    <p className={labelClass}>{t('table.recommendation')}</p>
                    <p>{issue.recommendation}</p>
                </div>
                <div className={`${baseMdClass} px-1 col-start-1 col-end-2 row-start-4`}>
                    <p className={statusStyles[issue.status]}>{t(`statuses.${issue.status}`)}</p>
                </div>
                <div className={`${baseMdClass} px-1 col-start-2 col-end-3 row-start-4`}>
                    <p className={riskLevelStyles[issue.riskLevel]}>{t(`riskLevels.${issue.riskLevel}`)}</p>
                </div>
                <div className={`${baseMdClass} px-1 col-start-1 col-end-3 row-start-6`}>
                    <p className={labelClass}>{t('table.responsibleUnit')}</p>
                    {issue.responsibleUnit.map((unit) => {
                        return <p className="text-xs md:mb-2">{unit}</p>
                    })} </div>
                <div className={`${baseMdClass} px-1 col-start-3 col-end-5 row-start-6`}>
                ...
                <div className={`${baseMdClass} px-1 col-start-1 col-end-3 row-start-7`}>
                    <p className={labelClass}>{t('table.scheduledDate')}</p>
                    <p> {issue.scheduledDate}</p>
                </div>
                <div className={`${baseMdClass} px-1 col-start-3 col-end-5 row-start-7`}>
                    <p className={labelClass}>{t('table.executionDate')}</p>
                    <p>{issue.executionDate === null ? '-' : issue.executionDate}</p></div>
            </div >
        })
